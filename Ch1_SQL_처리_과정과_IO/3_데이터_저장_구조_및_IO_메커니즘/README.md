...
중요한 것은 항상 Index가 유리한 것은 아니다. 

### 8. 캐시 탐색 메커니즘
Direct Path IO를 제외한 모든 블록IO는 메몰 버퍼캐시를 경유한다.
아래 오퍼레이션은 모두 버퍼캐시 탐색 과정을 수행한다.
+ 인덱스 루트 블록을 읽을 때
+ 인덱스 루트 블록에서 얻은 주소 정보로 브랜치 블록을 읽을 때
+ 인덱스 브랜치 블록에서 얻은 주소 정보로 리프 블록을 읽을 때
+ 인덱스 리프 블록에서 얻은 주소 정보로 테이블 블록을 읽을 때
...
실제론 훨씬 정교한 알고리즘이 작동하겠지만, 책에서 MOD를 이용한 해시 값으로 사용한다.
이처럼 15번 블록을 찾으면 해시함수를 이용해 해시체인을 찾는다. 그리고 그 포인터(버퍼 헤더)을 시작으로 15번 블록이 있는 곳을 찾는다. 
EX) 해시체인(0)->버퍼헤더(5)->버퍼헤더(25)->버퍼헤더(15)
이러한 방식의 특징은 다음과 같다.
1. 같은 입력 값은 항상 동일한 해시체인에 연결된다.
2. 다른 입력 값이 동일한 해시 체인에 연결될 수 있다.
3. 해시 체인 내에서는 정렬이 보장되지 않는다.

#### 메모리 공유자원에 대한 액세스 직렬화 
버퍼캐시는 SGA에 포함된다. 그러다보니 당연히 버퍼캐시에 캐싱된 버퍼블록은 모두 공유자원이다. 그렇다면 하나의 버퍼블록을 여러개의 프로세스가 동시에 접근하면 문제가 생길 수도있다. 그러다보니 순차적으로 접근할 수 있게 내부에선 직렬화(Serialization)매커니즘이 필요하다. 하나의 프로세스가 특정 공유자원을 사용할 때 다른 프로세스는 사용하지 못하고 기다리게 된다. 이러한 것을 가능하게 한 매커니즘이 래치이다.

SGA를 구서아는 서브 캐시마다 별도의 래치가 존대한다. 버퍼캐시에는 캐시버퍼체인 래치, 캐시버퍼 LRU 체인 래치 등이 작동한다.
앞서 말했듯이 버퍼캐시 히트율을 높이면 성능이 좋아지지만, 캐시IO도 이렇게 래치를 이용하다보니 래치끼리 경합을 하는 과정이 생겨 생각만큼 빠르지 않을수도 있다.
캐시버퍼 체인뿐만 아니라 버퍼블록 자체에도 Serialization 매커니즘이 존재한다. 바로 버퍼 Lock이다.
이런 **Serialization 매커니즘에 의한 캐시 경합을 줄이려면, SQL튜닝을 통해 쿼리 일량(논리적IO) 자체를 줄여야한다.**

+ 이 책에서 버퍼에 대한 Serialization 이 간략하게 어떻게 동작하는지 나와있지만 실제로는 내부에서는 매우 복잡하게 이루어진다. 단순히 데이터 조회뿐만 아니라 update,insert,delete에 상황에서도 Serialization이 지켜져야한다. 그러한 과정에서 DBMS는 데이터를 Lcok,unLock,Rollback,Commit 등 다양한 과정을 수행한다. Serialization이 지켜지고 데이터도 문제없게 유도해야한다.